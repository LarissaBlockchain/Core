...
var (
	...
	NetworkIdFlag = &cli.Uint64Flag{
		Name:     "networkid",
		Usage:    "Explicitly set network id (integer)(For testnets: use --neptune instead)",
		Value:    ethconfig.Defaults.NetworkId,
		Category: flags.EthCategory,
	}
	MainnetFlag = &cli.BoolFlag{
		Name:     "mainnet",
		Usage:    "Larissa mainnet",
		Category: flags.EthCategory,
	}
	NeptuneFlag = &cli.BoolFlag{
		Name:     "neptune",
		Usage:    "Neptune network: pre-configured proof-of-work test network",
		Category: flags.EthCategory,
	}
...
func MakeDataDir(ctx *cli.Context) string {
	if path := ctx.String(DataDirFlag.Name); path != "" {
		if ctx.Bool(NeptuneFlag.Name) {
			return filepath.Join(path, "neptune")
		}
		return path
	}
...
func setBootstrapNodes(ctx *cli.Context, cfg *p2p.Config) {
	...
	case ctx.Bool(NeptuneFlag.Name):
		urls = params.NeptuneBootnodes
	}
...
func SetDataDir(ctx *cli.Context, cfg *node.Config) {
	...
	case ctx.Bool(NeptuneFlag.Name) && cfg.DataDir == node.DefaultDataDir():
		cfg.DataDir = filepath.Join(node.DefaultDataDir(), "neptune")
	}
}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *ethconfig.Config) {
	...
	CheckExclusive(ctx, MainnetFlag, DeveloperFlag, NeptuneFlag)
	...
	case ctx.Bool(NeptuneFlag.Name):
		...
		cfg.Genesis = core.DefaultNeptuneGenesisBlock()
		SetDNSDiscoveryDefaults(cfg, params.NeptuneGenesisHash)
	case ctx.Bool(DeveloperFlag.Name):
		if !ctx.IsSet(NetworkIdFlag.Name) {
			cfg.NetworkId = 1337
		}
...
func MakeGenesis(ctx *cli.Context) *core.Genesis {
	...
	case ctx.Bool(NeptuneFlag.Name):
		genesis = core.DefaultNeptuneGenesisBlock()
	case ctx.Bool(DeveloperFlag.Name):
