...
func TestInvalidCliqueConfig(t *testing.T) {
	block := DefaultNeptuneGenesisBlock()
...
func TestSetupGenesis(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "custom block in DB, genesis == neptune",
			fn: func(db ethdb.Database) (*params.ChainConfig, common.Hash, error) {
				customg.MustCommit(db)
				return SetupGenesisBlock(db, trie.NewDatabase(db), DefaultNeptuneGenesisBlock())
			},
			wantErr:    &GenesisMismatchError{Stored: customghash, New: params.NeptuneGenesisHash},
			wantHash:   params.NeptuneGenesisHash,
			wantConfig: params.NeptuneChainConfig,
		},
		{
...
func TestGenesisHashes(t *testing.T) {
	for i, c := range []struct {
		...
		{DefaultNeptuneGenesisBlock(), params.NeptuneGenesisHash},
	} {
		// Test via MustCommit
